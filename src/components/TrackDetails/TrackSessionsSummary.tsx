    ...
    <div>
      ...
      <div className="space-y-4">
        {sessions.map((session, index) => {
          ...
          return (
            <div key={session.id} className="bg-gray-100 rounded-xl p-5 border border-gray-200 flex gap-4">
              <div className="mt-1">
                <Icon className="w-6 h-6 text-gray-700" />
              </div>
              <div>
                <h3 className="text-lg font-bold text-gray-900 mb-1">Session {session.id} : {session.title}</h3>
